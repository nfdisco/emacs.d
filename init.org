...
Use ido (Interactive Do) in the minibuffer.
...
Uncomment to avoid confirmation when creating new buffers -- only has
effect when ~confirm-nonexistent-file-or-buffer~ is ~nil~.
...
Bind ~C-p~ to ~previous-history-element~ for consistency -- also remap
