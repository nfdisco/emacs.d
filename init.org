...
Use ~TAB~ for completion, but only if the current line is already
indented.  Otherwise ~TAB~ indents the current line.
...
Once in the completion list buffer, use ~p~ and ~n~ to go to the previous
and next item.
