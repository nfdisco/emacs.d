...
Do not keep duplicated entries in the minibuffer history.
...
#+BEGIN_SRC emacs-lisp
(let ((variables '(search-ring regexp-search-ring)))
  (if (boundp 'savehist-additional-variables)
      (dolist (var variables)
        (add-to-list savehist-additional-variables var))
    (setq savehist-additional-variables variables)))
#+END_SRC
...
Enable file name and buffer name completion with ido.
...
In the completion list buffer, type ~p~ and ~n~ to walk through the
