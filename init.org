...
Store custom themes in the ~themes/~ directory under the user's Emacs
directory.
...
Add any subdirectory under the themes directory to the theme search
path.  Subdirectories whose names start with a dot are ignored.

#+BEGIN_SRC emacs-lisp
(when (and (boundp 'custom-theme-load-path)
           (file-directory-p custom-theme-directory))
  (dolist (f (directory-files custom-theme-directory t "^[^.]"))
    (when (file-directory-p f)
      (add-to-list 'custom-theme-load-path (file-name-as-directory f)))))
#+END_SRC

*** Solarized

Uncomment to add the theme's subdirectory to the search path.
